...
const getAllDirections = async (): Promise<Direction[] | null> => {
  try {
    ...
    return response.data.code === 1000
      ? response.data.result.map((direction: any) => ({
          id: direction.id,
...
const getAllNumbers = async (): Promise<Number[] | null> => {
  try {
    ...
    return response.data.code === 1000
      ? response.data.result.map((number: any) => ({
          id: number.id,
...
const getAllShapes = async (): Promise<Shape[] | null> => {
  try {
    ...
    return response.data.code === 1000
      ? response.data.result.map((shape: any) => ({
          id: shape.id,
          shape: shape.shape,
        }))
      : null;
...
const getAllAnimalCategory = async (): Promise<AnimalCategory[] | null> => {
  try {
    ...
    return response.data.code === 1000
      ? response.data.result.map((animalCategory: any) => ({
...
const CompatibilityForm = () => {
  const [caculationData, setCaculationData] =
    useState<CompatibilityResultResponse | null>(null);
  ...
  const handleAnimalChange = (animalId: number, animalName: string) => {
    if (
      ...
      !selectedAnimals.some((animal) => animal.animalId === animalId)
    ) {
  ...
  const removeAnimal = (animalId: number) => {
    setSelectedAnimals((prev) =>
      prev.filter((animal) => animal.animalId !== animalId)
    );
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await getCompatibilityResult(
      parsedYear,
  ...
  return (
    <div className="background-fenghsui">
      <form onSubmit={handleSubmit} className="compatibility-form">
        ...
        <h1 className="compa-form-title">
          CHECK FENG SHUI COMPATIBILITY OF KOI POND BASED ON THE FIVE ELEMENTS
        </h1>
                          ...
                          ?.filter(
                            (ac) =>
                              !selectedAnimals.some(
                                (sa) => sa.animalId === ac.id
                              )
                          )
                          .map((ac) => (
                            <option key={ac.id} value={ac.id}>
                              {ac.animalCategoryName}
                            </option>
                          ))}
                      </select>
                    )}
                  ...
                  <div>
                    {selectedAnimals?.map((animal) => (
                      <div className="animal-picked" key={animal.animalId}>
                        {animal.animalName}
                        <button
                          className="Compa-x-button"
                          type="button"
                          onClick={() => removeAnimal(animal.animalId)}
                        >
                          X
                        </button>
                      </div>
        ...
        <button type="submit" className="compability-btn">
          Check compatibility
        </button>
      </form>
      <p className="notify-login-blog">
        *For 11 or more koi fish, disregard the tens digit and use the units
        digit. For example: 11, 15, 38, 40, 49 fish... count as 1, 5, 8, 4, 9
        fish. Then refer to the table above for calculation.
      </p>
        ...
        <div className="result-container">
          ...
          {caculationData.directionExplanation && (
            <div className="direction">
              ...
              {caculationData.directionName && (
                <p className="direction-name">
                  Selected direction: {caculationData.directionName}
                </p>
              )}
              <p className="direction-explanation">
                <div className="explain">Explain:</div>{" "}
                {caculationData.directionExplanation}
              </p>
              {caculationData.directionScore !== null && (
                <p className="direction-score">
                  Score: {caculationData.directionScore}/5
                </p>
              )}
              {caculationData.directionsAdvice &&
                caculationData.directionsAdvice.size > 0 && (
                  <p className="directions-advice">
                    Better Directions:{" "}
                    {Array.from(caculationData.directionsAdvice).join(", ")}
                  </p>
                )}
            </div>
          )}
          ...
          {caculationData.numberExplanation && (
            <div className="number">
              ...
              {caculationData.number && (
                <p className="number">
                  Selected number: {caculationData.number}
                </p>
              )}
              <p className="number-explanation">
                <div className="explain">Explain:</div>{" "}
                {caculationData.numberExplanation}
              </p>
              {caculationData.numberScore !== null && (
                <p className="number-score">
                  Score: {caculationData.numberScore}/5
                </p>
              )}
              {caculationData.numbersAdvice &&
                caculationData.numbersAdvice.size > 0 && (
                  <p className="numbers-advice">
                    Better Numbers:{" "}
                    {Array.from(caculationData.numbersAdvice).join(", ")}
                  </p>
                )}
            </div>
          )}
          ...
          {caculationData.shapeExplanation && (
            <div className="shape">
              ...
              {caculationData.shapeName && (
                <p className="shape-name">
                  Selected shape: {caculationData.shapeName}
                </p>
              )}
              <p className="shape-explanation">
                <div className="explain">Explain:</div>{" "}
                {caculationData.shapeExplanation}
              </p>
              {caculationData.shapeScore !== null && (
                <p className="shape-score">
                  Score: {caculationData.shapeScore}/5
                </p>
              )}
              {caculationData.shapesAdvice &&
                caculationData.shapesAdvice.size > 0 && (
                  <p className="shapes-advice">
                    Better Shapes:{" "}
                    {Array.from(caculationData.shapesAdvice).join(", ")}
                  </p>
                )}
            </div>
          )}
          {caculationData.animalCompatibilityResponse &&
            caculationData.animalCompatibilityResponse.length > 0 && (
              <div className="animal">
                <h3 className="animal-title">Fish</h3>
                {caculationData.animalCompatibilityResponse.map(
                  (animal, index) => (
                    <div key={index} className="animal-info">
                      {animal.animalName && (
                        <p className="animal-name">
                          Selected fish: {animal.animalName}
                        </p>
                      )}
                      {animal.animalScore !== null && (
                        <p className="animal-colors">
                          {animal.animalName} colors:{" "}
                          {animal.animalColors.join(", ")}
                        </p>
                      )}
                      {animal.colorCompatibilityResponses && (
                        <p className="animal-explanation">
                          ...
                          <ul className="color-explanations">
                            {animal.colorCompatibilityResponses.map(
                              (response, index) => (
                      ...
                      {animal.animalScore !== null && (
                        <p className="animal-score">
                          Score: {animal.animalScore}/5
                        </p>
                      )}
                    </div>
                  )
                ...
                {caculationData.animalAverageScore !== null && (
                  <p className="animal-average-score">
                    Average Score: {caculationData.animalAverageScore}/5
                  </p>
                )}
                {caculationData.animalAdvice &&
                  caculationData.animalAdvice.size > 0 && (
                    <p className="animal-advice">
                      Better Fish Choices:{" "}
                      {Array.from(caculationData.animalAdvice).join(", ")}
                    </p>
                  )}
              </div>
            )}
