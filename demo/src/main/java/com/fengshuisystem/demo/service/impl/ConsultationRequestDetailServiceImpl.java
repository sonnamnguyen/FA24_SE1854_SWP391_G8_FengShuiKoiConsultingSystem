...
public class ConsultationRequestDetailServiceImpl implements ConsultationRequestDetailService {
    ...
    public ConsultationRequestDetail save(ConsultationRequestDetailDTO dto) {
        ...
        // Lấy các ShelterCategory từ DB theo ID và chuyển thành Set
        Set<ShelterCategory> shelterCategories = new HashSet<>(
                shelterCategoryRepository.findAllById(dto.getShelterCategoryIds())
        );

        // Lấy các AnimalCategory từ DB theo ID và chuyển thành Set
        Set<AnimalCategory> animalCategories = new HashSet<>(
                animalCategoryRepository.findAllById(dto.getAnimalCategoryIds())
        );
